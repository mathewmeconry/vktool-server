...
describe('BillingReportController', function () {
	...
	before(() => {
		...
		//@ts-ignore
		report.compensationEntries[TestHelper.mockContact.id] = Object.assign(
			{},
			TestHelper.mockContact,
			{
	...
	it('should create a new billing report', async () => {
		return supertest(app)
			...
			.then((res) => {
				...
				//@ts-ignore
				expect(reportres.compensations[0].from).to.be.equal(
					report.compensationEntries[TestHelper.mockContact.id].from
				);
				//@ts-ignore
				expect(reportres.compensations[0].until).to.be.equal(
					report.compensationEntries[TestHelper.mockContact.id].until
				);
				//@ts-ignore
				expect(reportres.compensations[0].charge).to.be.equal(
					report.compensationEntries[TestHelper.mockContact.id].charge
				);
