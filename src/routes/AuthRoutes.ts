...
export default function AuthRoutes(app: Express.Application) {
    ...
    if (process.env.TESTING) app.get('/api/auth/mock-admin', passport.authenticate('mock-admin'), (req, res) => { res.send(req.user) })
    if (process.env.TESTING) app.get('/api/auth/mock', passport.authenticate('mock-nonadmin'), (req, res) => { res.send(req.user) })
