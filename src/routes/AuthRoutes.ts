...
export default function AuthRoutes(app: Express.Router) {
	...
	if (process.env.TESTING)
		app.get('/auth/mock-admin', passport.authenticate('mock-admin'), (req, res) => {
			res.send(req.user);
		});
	if (process.env.TESTING)
		app.get('/auth/mock', passport.authenticate('mock-nonadmin'), (req, res) => {
			res.send(req.user);
		});
	if (process.env.TESTING)
		app.get(
			'/check/authservice',
			AuthService.checkAuthorization([AuthRoles.MEMBERS_READ]),
			(req, res) => {
				res.status(200);
				res.send('');
			}
