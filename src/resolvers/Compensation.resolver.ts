...
import { Resolver, FieldResolver, Root, InputType, Field, Mutation, Arg, Ctx, Authorized, Query, registerEnumType } from 'type-graphql'
...
export default class CompensationResolver extends baseResolver {
	...
	public async getUserCompensations(@Arg('id') id: number, @Ctx() ctx: ApolloContext): Promise<Compensation<any>[]> {
		if (ctx.user.id !== id && !AuthService.isAuthorized(ctx.user.roles, AuthRoles.COMPENSATIONS_READ)) {
			throw new Error('Access denied! You don\'t have permission for this action!')
		}

		const customs = await getManager().getRepository(CustomCompensation).find({ memberId: id })
		const orders = await getManager().getRepository(OrderCompensation).find({ memberId: id })
