import { Resolver, Root, FieldResolver, InputType, Field, Mutation, Arg, Ctx, Args } from "type-graphql"
...
export default class PayoutResolver extends baseResolver {
    ...
    public async transferPayout(@Arg('id') id: number, @Arg('memberIds', type => [Number]) memberIds: number[], @Ctx() ctx: ApolloContext): Promise<boolean> {
    ...
    @Mutation(type => Boolean)
    public async send2Bexio(@Arg('id') id: number, @Arg('memberIds', type => [Number], { nullable: true }) memberIds?: number[]): Promise<boolean> {
        const payout = await resolveEntity<Payout>('Payout', id)
    ...
    @Mutation(type => Boolean)
    public async sendMemberEmails(@Arg('id') id: number, @Arg('memberIds', type => [Number], { nullable: true }) memberIds?: number[]): Promise<boolean> {
        const payout = await resolveEntity<Payout>('Payout', id)
        let toSendIds = memberIds
        if (!toSendIds) {
            toSendIds = payout.compensations.map(r => r.memberId)
    ...
    public async reclaimPayout(@Arg('id') id: number): Promise<Payout> {
        ...
        const completePayout = await getManager().getRepository(Payout).findOne(payout.id, {
            join: {
                alias: 'payout',
                leftJoinAndSelect: {
    ...
    @Mutation(type => Payout)
    public async addPayout(@Arg('until') until: Date, @Arg('from', { nullable: true }) from?: Date): Promise<Payout> {
    ...
    @FieldResolver()
    public async compensations(@Root() object: Payout): Promise<Array<OrderCompensation | CustomCompensation>> {
        const customs = await resolveEntityArray<CustomCompensation>('CustomCompensation', object.compensationIds)
        const orders = await resolveEntityArray<OrderCompensation>('OrderCompensation', object.compensationIds)
