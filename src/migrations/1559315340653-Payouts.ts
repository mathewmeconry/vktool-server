...
export class Payouts1559315340653 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<any> {
        await queryRunner.query("ALTER TABLE `payout` DROP COLUMN `date`");
        await queryRunner.query("ALTER TABLE `payout` ADD `until` date NOT NULL");
        await queryRunner.query("ALTER TABLE `payout` ADD `from` date NOT NULL");
    ...
    public async down(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query("ALTER TABLE `compensation` CHANGE `updatedAt` `updatedAt` timestamp(6) NULL DEFAULT 'current_timestamp(6)'");
        await queryRunner.query("ALTER TABLE `payout` CHANGE `updatedAt` `updatedAt` timestamp(6) NULL DEFAULT 'current_timestamp(6)'");
        await queryRunner.query("ALTER TABLE `payout` DROP COLUMN `from`");
        await queryRunner.query("ALTER TABLE `payout` DROP COLUMN `until`");
        await queryRunner.query("ALTER TABLE `payout` ADD `date` date NOT NULL");
    }
