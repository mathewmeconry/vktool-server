...
export class TransferCompensation1566402721170 implements MigrationInterface {
	public async up(queryRunner: QueryRunner): Promise<any> {
		...
		await queryRunner.query(
			'ALTER TABLE `compensation` ADD CONSTRAINT `FK_7f193f4248bc7ef48a77f7a6e5c` FOREIGN KEY (`transferCompensationId`) REFERENCES `compensation`(`id`)'
		);
	}

	public async down(queryRunner: QueryRunner): Promise<any> {
		await queryRunner.query(
			'ALTER TABLE `compensation` DROP FOREIGN KEY `FK_7f193f4248bc7ef48a77f7a6e5c`'
		);
		await queryRunner.query(
			"ALTER TABLE `billing_report` CHANGE `updatedAt` `updatedAt` timestamp(6) NULL DEFAULT 'current_timestamp(6)'"
		);
