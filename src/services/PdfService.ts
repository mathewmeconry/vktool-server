...
export default class PdfService {
    public static async generateMemberPayout(payout: Payout, member: Contact, compensations: Array<CustomCompensation | OrderCompensation>, compensationTotal: number): Promise<Buffer> {
        const logo = await fs.readFile(path.resolve(__dirname, '../../public/logo.png'))
        return PdfService.generatePdf('memberPayout.pug',
            {
    ...
    public static async generateMemberList(members: Contact[]): Promise<Buffer> {
        const logo = await fs.readFile(path.resolve(__dirname, '../../public/logo.png'))
        return PdfService.generatePdf('memberList.pug',
            {
            ...
            },
            {
                ...
                displayHeaderFooter: true,
                headerTemplate: pug.renderFile(path.resolve(__dirname, '../../public/pdfs/pugs/header.pug'), { logo: `data:image/png;base64,${logo.toString('base64')}` }),
                footerTemplate: pug.renderFile(path.resolve(__dirname, '../../public/pdfs/pugs/footer.pug'), { width: '190mm' }),
                format: 'A4',
                landscape: true,
                margin: {
                    top: '25mm',
