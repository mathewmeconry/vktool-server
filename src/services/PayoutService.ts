...
export default class PayoutService {
	public static async reclaimCompensations(payout: Payout): Promise<void> {
		...
		await qb
			.update()
			.set({ payout: undefined })
			.where('payout = :payout', { payout: payout.id })
			.execute();
	...
	public static async generateOverviewPDF(payout: Payout): Promise<Buffer> {
		...
		if (payout.compensations && payout.compensations.length > 0) {
			...
			payout.compensations.forEach((el) => {
				if (!aggregated.hasOwnProperty((el.memberId || 0).toString()))
					aggregated[(el.memberId || 0).toString()] = [] as Compensation<any>[];
				aggregated[(el.memberId || 0).toString()].push(el);
			});

			for (const memberId in aggregated) {
				data.push(
					Object.assign(aggregated[memberId][0].member, {
						amount: aggregated[memberId].reduce((total, comp) => {
	...
	public static async generateMemberMail(payout: Payout, member: Contact): Promise<string> {
		return new Promise<string>((resolve, reject) => {
			fs.readFile(path.resolve(__dirname, '../../public/logo.png'), async (err, data) => {
				...
				resolve(
					pug.renderFile(
						path.resolve(__dirname, '../../public/emails/memberPayout/memberPayout.pug'),
						{
							apiEndpoint: config.get('apiEndpoint'),
							compiledStyle: sass.renderSync({
								file: path.resolve(__dirname, '../../public/emails/memberPayout/memberPayout.scss'),
							}).css,
	...
	public static async sendMemberToBexio(payout: Payout, member: Contact): Promise<void> {
		...
		const bill = await BexioService.createBill(
			positions,
			member,
			true,
			`Entschädiungsauszahlung #${payout.id} ${moment(payout.until).format('DD.MM.YYYY')}`,
			`Entschädiungsauszahlung #${payout.id} ${moment(payout.until).format('DD.MM.YYYY')}`,
			true
		);
			...
			xmlFile.compile((err: Array<string>, out: string) => {
				if (err && (err.length > 1 || err[0] !== 'The list of transactions is empty.')) {
					...
					resolve(
						(out || '')
							.replace(/"EUR"/g, '"CHF"')
							.replace(/<PmtTpInf>.*<\/PmtTpInf>/gms, '')
							.replace(
								/<EndToEndId>/g,
								() =>
									`<InstrId>${(Math.random() * 10000)
										.toString()
										.replace('.', '')}</InstrId><EndToEndId>`
							)
							.replace(
		...
		for (const memberId of memberIds) {
			...
			if (compensations.length > 0) {
				...
				const transferCompensation = new CustomCompensation(
					...
					total,
					new Date(),
					`Übertrag der Entschädiungsperiode ${
						payout.from > new Date('1970-01-01')
							? `${moment(payout.from).format('DD.MM.YYYY')} -`
							: ''
					}${moment(payout.until).format('DD.MM.YYYY')}`,
					true
				);
