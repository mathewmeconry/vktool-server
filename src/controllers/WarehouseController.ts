...
export default class WarehouseController {
	...
	public static async getFinanceReport(req: Express.Request, res: Express.Response): Promise<void> {
		const warehouses = await getManager().getRepository(Warehouse).find({});

		let allEntries: Array<StockEntry> = [];

		for (const warehouse of warehouses) {
			if (warehouse.name !== 'Defekt') {
				const entries = await MaterialChangelogService.getWarehouseStock(warehouse.id);
				allEntries = allEntries.concat(entries);
			}
		}
		...
		res.send(await PdfService.generateWarehousesFinanceReport(allEntries.filter((e) => e.amount != 0)));
	}
